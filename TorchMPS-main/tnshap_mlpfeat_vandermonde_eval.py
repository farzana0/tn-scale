#!/usr/bin/env python3
"""
tnshap_mlpfeat_vandermonde_eval.py

TN-SHAP-style Shapley computation for a teacher and its MPS+MLP surrogate:

  - Diagonal evaluations in scalar parameter t
  - Chebyshev interpolation nodes in [0, 1]
  - Vandermonde system (square) to recover degree-wise aggregates
  - Recurrence to extract per-feature degree aggregates β_{i,k}
  - Shapley_i = sum_k β_{i,k} / k

We compare:

  - TN-SHAP on the true teacher (paths in ORIGINAL x-space: t * x)
  - TN-SHAP on the trained MPS+MLP surrogate, evaluated on the same
    ORIGINAL paths:
      - h(t): MPS(t * x)
      - g_i(t): MPS(x_i fixed, t * x_{-i})

This is generic: use it for either
  - degree-10 polynomial teacher, or
  - squared-exponential teacher y = exp(sum_{i in S} x_i^2)
by choosing the right prefix and max_degree.
"""
...
# Scalar MLP feature map: must match training script
...
# -----------------------
# Diagonal evals in ORIGINAL x-space for MPS+MLP
# -----------------------

def eval_h_on_nodes_mps(mps: MPS, x: torch.Tensor, t_nodes: torch.Tensor) -> torch.Tensor:
    """
    h(t) for MPS+MLP in ORIGINAL space:
      - input to MPS is s = t * x
      - MPS internally applies phi_scalar(s_i) with MLP, then appends 1.
    """
    x = x.to(DEVICE)
    t_nodes = t_nodes.to(x.device)
...
def eval_g_i_on_nodes_mps(mps: MPS, x: torch.Tensor, i: int, t_nodes: torch.Tensor) -> torch.Tensor:
    """
    g_i(t) for MPS+MLP in ORIGINAL space:
      - s_j(t) = t * x_j for j != i
      - s_i(t) = x_i  (clamped)
    """
    x = x.to(DEVICE)
...
# -----------------------
# Load MPS+MLP with correct feature map
# -----------------------

def load_mps(prefix: str):
    state = torch.load(f"{prefix}_mps_mlpfeat.pt", map_location=DEVICE)
    input_dim = state["input_dim"]
    bond_dim = state["bond_dim"]
    mlp_hidden = state.get("mlp_hidden", 32)
    fmap_name = state.get("feature_map", "scalar_mlp3_plus_one")
    ...
    mlp_feat = ScalarMLPFeatureMap(hidden_dim=mlp_hidden).to(DEVICE)
...
def tnshap_vandermonde_mps(
    ...
    """
    TN-SHAP for the MPS+MLP surrogate, with paths in ORIGINAL x-space:
      - h(t): MPS(t * x)
      - g_i(t): MPS(x_i fixed, t * x_{-i})
    """
    x = x.to(DEVICE)
...
def main():
    ...
    parser.add_argument("--prefix", type=str, default="poly10_D50",
                        help="Prefix for *_teacher.pt, *_tnshap_targets.pt, *_mps_mlpfeat.pt")
    ...
    prefix = args.prefix
    print(f"=== TN-SHAP MLP-feature Vandermonde Eval for PREFIX={prefix} ===")
    ...
    fmap_name_targets = targets.get("feature_map", "scalar_mlp3_plus_one")
