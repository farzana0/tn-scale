...
# Training parameters (mask space, not MNIST)
...
########################################
# Hadamard-like feature map
########################################

def random_orthogonal(n: int, device: torch.device) -> torch.Tensor:
    """
    Build a random orthogonal matrix Q in R^{n x n} via QR.
    """
...
# Fixed orthogonal matrix H: R^{n_sites x n_sites}
H = random_orthogonal(n_sites, device=DEVICE)
...
def build_dataset(alpha: float, num_test: int):
    ...
    Each sample: (features, label) where
      - we first draw a random mask S
      - we encode z in {-1, +1}^n
      - we apply Hadamard-like map: u = H z
      - we feed u to the MPS, and the label is DistilBERT logit f(S)
    """
    ...
    for _ in range(L):
        ...
        # apply H: (1, n) @ (n, n) -> (1, n)
        z_t = torch.tensor(z, device=DEVICE).unsqueeze(0)    # (1, n)
        u_t = torch.matmul(z_t, H.T)                         # (1, n)
        u = u_t.squeeze(0).cpu().numpy().astype(np.float32)  # (n,)
    ...
    for _ in range(num_test):
        ...
        z_t = torch.tensor(z, device=DEVICE).unsqueeze(0)    # (1, n)
        u_t = torch.matmul(z_t, H.T)
        u = u_t.squeeze(0).cpu().numpy().astype(np.float32)
...
mps = MPS(
    input_dim=n_sites,          # features dimension = n_sites (after Hadamard)
...
print(
    f"Training MPS on DistilBERT mask surrogate (Hadamard features) \n"
...
for epoch_num in range(1, num_epochs + 1):
    ...
    for inputs, labels in train_loader:
        inputs = inputs.to(DEVICE)   # (B, n_sites)
        labels = labels.to(DEVICE)   # (B,)

        scores = mps(inputs).squeeze(-1)  # (B,)
