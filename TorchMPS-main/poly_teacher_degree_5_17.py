...
def make_sparse_poly_coeffs(
    ...
    # Active subset of features (first one-third in the PKeX setting)
...
def load_teacher(prefix: str) -> SparsePolyTeacher:
    ...
    a = state["a"]
    b = state["b"]
    idx3 = state.get("idx3", torch.empty(0, 3, dtype=torch.long))
    c3 = state.get("c3", torch.empty(0, dtype=torch.float32))
    idx4 = state.get("idx4", torch.empty(0, 4, dtype=torch.long))
    c4 = state.get("c4", torch.empty(0, dtype=torch.float32))
    idx5 = state.get("idx5", torch.empty(0, 5, dtype=torch.long))
...
def main():
    ...
    parser.add_argument("--D", type=int, default=50)          # R^50
    parser.add_argument("--n-train", type=int, default=1000)  # n = 1000 samples
    parser.add_argument("--n-test", type=int, default=1000)
    parser.add_argument("--k-active", type=int, default=17)   # |S| = 17 (first one-third)
