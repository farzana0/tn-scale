#!/usr/bin/env python3
"""
local_neighborhood_vs_coalitions_treeshap.py

Compare 4 local sampling schemes around each evaluation point x0:

  1) PATH       : x(t)      = t * x0
  2) COAL       : x(m)      = m ⊙ x0, m ∈ {0,1}^D
  3) RANDOM     : x_rand    = x0 + ε, ε ~ N(0, σ^2 I)
  4) TNSHAP_GI  : clean TN-SHAP-style g_i(0,t), g_i(q,t):
...
def set_global_seed(seed: int):
    ...
    # For extra determinism in case any cudnn ops are used
...
def build_random_dataset(x0, teacher, n_points, sigma):
    """
    RANDOM scheme:
      - X = x0 + ε, ε ~ N(0, diag(sigma^2))
    sigma: torch tensor of shape (D,)
    """
    D = x0.shape[0]
    noise = torch.randn(n_points, D, device=x0.device) * sigma.unsqueeze(0)
    X = x0.unsqueeze(0) + noise
    with torch.no_grad():
        y = teacher(X).view(-1)
    return X.cpu().numpy(), y.cpu().numpy()
...
def train_and_eval_models(X_train, y_train, x0_np, support_true, support_size):
    ...
    # XGBoost (make it deterministic-ish)
    xgb = XGBRegressor(
        n_estimators=200,
        max_depth=4,
        learning_rate=0.05,
...
def main():
    ...
    parser.add_argument("--prefix", type=str, required=True,
                        help="Task prefix, e.g. poly5_D50, poly5_D100, poly10_D50, poly10_D100, sqexp_D50, sqexp_D100.")
    ...
    # For RANDOM scheme: per-feature std, small fraction
    X_std = X_all.std(dim=0)
    sigma = 0.1 * torch.clamp(X_std, min=1e-6)
    ...
    for it, idx in enumerate(idx_eval, start=1):
        ...
        n_train = 2 * D  # total samples for PATH / COAL / RANDOM; TNSHAP_GI naturally has 2D
        ...
        X_rand, y_rand = build_random_dataset(
            x0=x0, teacher=teacher,
            n_points=n_train, sigma=sigma
        )
        res_rand = train_and_eval_models(X_rand, y_rand, x0_np, support_true, args.support_size)
        for m in models:
            r2_scores["RANDOM"][m].append(res_rand[m][0])
            acc_scores["RANDOM"][m].append(res_rand[m][1])
        ...
        print("  RANDOM    : " + ", ".join([f"{m}: R2={res_rand[m][0]:.3f}, acc={res_rand[m][1]:.3f}" for m in models]))
